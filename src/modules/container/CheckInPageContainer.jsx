...
import {
  ...
  updatePassengerSeatAndStatus,
  updatePassengerDetail,
  createNewPassenger
} from "../../axios/PassengerAPI";
...
export class CheckInPageContainer extends Component {
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  componentDidMount() {
    let passengerListPromises = fetchFlightPassengers(
      this.props.match.params.id
    );
    passengerListPromises
      .then(passengerList => {
        this.setState({
      ...
      .then(passengerList => {
        ...
        for (let i = 0; i < passengerList.length; i++) {
          if (passengerList[i].status === "AC") {
            ...
            let seatNo = this.getSeatNoFromSeatLocation(
    ...
    flightPromises.then(flight =>
      this.setState({ flight: flight, items: flight.special_meals })
    );
  }
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  updateHandler = event => {
    ...
    let passengerDetailPromise = updatePassengerDetail(
      selectedPassengerId,
      data
    );
    passengerDetailPromise.then(response => {
      ...
      let index = passengerList.findIndex(
        passenger => passenger.id === Number(selectedPassengerId)
      );
  ...
  getPassengerId = event => {
    ...
    let passenger = this.state.allPassengerList.find(
    ...
    if (!this.state.isAdmin) {
      ...
      for (let i = 0; i < passenger.special_meals.length; i++) {
        let index = flightSpecialMeals.findIndex(
          item => passenger.special_meals[i] === item
        );
        passengerMealItem[index] = true;
      }
      this.setState({
        passengerMealItem: passengerMealItem,
        passengerOldMealItem: passengerMealItem
      });
      ...
      for (let i = 0; i < passenger.ancillary.length; i++) {
        let index = flightAncillary.findIndex(
          item => passenger.ancillary[i] === item
        );
        passengerAncillary[index] = true;
      }
      this.setState({
        passengerAncillaryService: passengerAncillary,
        passengerOldAncillaryService: passengerAncillary
      });
    }
  };
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  updatePassengerList = (paxId, seatLocation, passengerList, seatMap) => {
    ...
    let updatedPaxDetailPromises = updatePassengerSeatAndStatus(
      passengerList[i].id,
      data
    );
  ...
  handleClick = event => {
    ...
    let updatedPaxList = this.updatePassengerList(
      paxId,
      seatLocation,
      passengerList,
      seatMap
  ...
  handleSubmit = event => {
    ...
    flightPromises.then(response =>
      console.log("Flight Meal or Ancillary Updated")
    );
  };
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  handleChange = event => {
  ...
  updateToPreviousAncillaryServices = () => {
    ...
    this.setState({
      passengerAncillaryService: this.state.passengerOldAncillaryService
    });
  };
          ...
          <MDBRow>
            <MDBCol md='8'>
              <Passenger
                ...
                passengerId={this.state.selectedPassengerId}
              ...
              >
                <MDBCol>
                  {this.state.isAdmin ? (
                    <NewPassenger
                      flightId={flightId}
                      selectGender={event => this.selectGender(event)}
                      hasInfant={event => this.hasInfant(event)}
                      wheelChairRequired={event =>
                        this.wheelChairRequired(event)
                      }
                ...
                <MDBCol>
                  {this.state.isAdmin ? (
                    <UpdatePassengerDetail
                      ...
                      submitHandler={event => this.updateHandler(event)}
                      changeHandler={event => this.changeHandler(event)}
                      passengerDetail={this.state}
                    />
                  ) : (
                ...
                <MDBCol>
                  {!this.state.isAdmin ? (
                    <MealServices
                      state={this.state}
                      updatePassengerMealItem={event =>
                        this.updatePassengerMealItem(event)
                      }
                      updateToPreviousMealItem={() =>
                        this.updateToPreviousMealItem()
                      }
                    />
                  ) : (
                    <></>
                ...
                <MDBCol>
                  {!this.state.isAdmin ? (
                    <AncillaryServices
                      state={this.state}
                      updatePassengerAncillaryService={event =>
                        this.updatePassengerAncillaryService(event)
                      }
                      updateToPreviousAncillaryServices={() =>
                        this.updateToPreviousAncillaryServices()
                      }
                    />
                  ) : (
                    <></>
            ...
            <MDBCol md='4'>
              {!this.state.isAdmin ? (
                <SeatContainer
                  ...
                  bookedSeatMap={this.state.seatMap}
                />
              ) : (
                <>
